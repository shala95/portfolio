...
DATA_DIR = PROJECT_ROOT / "data" / "Data_to_be_fetched"  # Put your CSV file here
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "fetched_data"

# API configuration - Using direct IP address with hostname in header
# API configuration - Using direct IP address with hostname in header
IP_ADDRESS_PROD = "34.249.81.174"
IP_ADDRESS_STAGE = "3.249.199.236"
PORT = 7106
API_HOSTNAME = "xseed-advanced-metrics-btb-env.eba-p2hm3mer.eu-south-1.elasticbeanstalk.com"
API_ENDPOINT = f"http://{IP_ADDRESS_PROD}:{PORT}/trace"  # Default to production
TIMEOUT = 90
MAX_RETRIES = 3
RETRY_BACKOFF = 1
...
def get_trace_data(user_id, session_id, segment_id, trace_type="match", version=1, output_file=None, environment="prod"):
    """
    ...
    Args:
        ...
        environment (str): Environment to use, "prod" or "stage" (default: "prod")
    ...
    # Select the appropriate IP address based on environment
    ip_address = IP_ADDRESS_PROD if environment.lower() == "prod" else IP_ADDRESS_STAGE
    api_endpoint = f"http://{ip_address}:{PORT}/trace"
    ...
    try:
        logger.info(f"Fetching data for user={user_id}, session={session_id}, segment={segment_id} from {environment} environment")
...
def fetch_player_data(row, output_dir, trace_type="match", version=1, environment="prod"):
...
def fetch_all_players_data(input_csv, output_dir, trace_type="match", version=1, max_workers=5, environment="prod"):
    """
...
def main():
    ...
    # Configure paths
    input_csv = DATA_DIR / "698_data_for_MS.csv"
    ...
    environment = "prod"  # Default to production environment, can be set to "stage"
